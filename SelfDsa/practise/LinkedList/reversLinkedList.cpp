...
node *kReversALinkList(node *head, int k)
{
    if (head == NULL)
    {
        return NULL;
...
int main()
{
    ...
    // printLinkList(rev);
    // cout << "Middle Value : " << middleOfLinkList(head)->data << endl;
    // cout << "SUM of All Data : " << SumOfAllData(head) <<endl;
    node *kRevGroup = kRevers(head, 3);
    printLinkList(kRevGroup);
    return 0;
}
